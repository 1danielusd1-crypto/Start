...
def financial_view_total_balance(store: dict | None) -> float:
    # v194: ARS reads the same daily index as the main/day/remaining windows.
    # USD remains transaction-ledger based.
    if not financial_view_is_usd(store) and "_ars_balance_from_daily_index_v194" in globals():
        try:
            return float(_ars_balance_from_daily_index_v194(store or {}) or 0)
        except Exception:
            pass
    total = 0.0
...
def financial_view_balance_through_day(store: dict | None, day_key: str) -> float:
    if not financial_view_is_usd(store):
        total = 0.0
        for dk in sorted(((store or {}).get("daily_records", {}) or {}).keys()):
            if str(dk) > str(day_key):
                break
            for rec in ((store or {}).get("daily_records", {}) or {}).get(dk, []) or []:
