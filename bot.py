...
def send_and_auto_delete(chat_id: int, text: str, delay: int = 10):
    """
    Отправка сообщения с автоудалением через заданное число секунд.
    Используется, например, в require_finance().
    """
    try:
        msg = bot.send_message(chat_id, text)
    except Exception as e:
        log_error(f"send_and_auto_delete send error: {e}")
        return
...
def handle_text(msg):
    ...
    try:
        ...
        # ----------------------------------------------
        # Forwarding (без петель: не пересылаем сообщения от ботов)
        # ----------------------------------------------
        targets = []
        try:
...
def handle_media(msg):
    ...
    try:
        ...
        # пересылка (без петель: не пересылаем сообщения от ботов)
        targets = []
        try:
            if not (msg.from_user and msg.from_user.is_bot):
                targets = resolve_forward_targets(chat_id)
...
def restore_file_switch(chat_id: int, file_id: str, file_name: str):
    ...
    try:
        ...
        # Глобальный data.json (весь словарь data)
        ...
        # Локальный JSON отдельного чата, созданный через /json
            ...
            try:
                store = json.loads(downloaded.decode("utf-8"))
                if cid is not None:
                    with data_lock:
                        data.setdefault("chats", {})[str(cid)] = store
                    save_data(data)
                bot.send_message(chat_id, f"✔️ Восстановлено: {file_name}")
            except Exception as e:
                log_error(f"restore_file_switch JSON per-chat error: {e}")
                bot.send_message(chat_id, "❌ Ошибка восстановления JSON файла.")

        # CSV отдельного чата (просто сохраняем на диск)
