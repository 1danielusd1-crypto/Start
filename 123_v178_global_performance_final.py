...
def heavy_processes_status_v175() -> str:
    ...
    return f"⚙️ Центр процессов v178: отключено {off} из {len(_V176_PROCESS_DEFS)} управляемых процессов."
    ...
    for handler in list(getattr(bot, "callback_query_handlers", []) or []):
        ...
        def _wrapped(call, _orig=original):
            ...
            try:
                ...
                action = raw
            _V177_PERF_LOCAL.action = action[:120]
            try:
                return _orig(call)
            finally:
                try:
...
try:
    _V176_ORIG_BOT_JOURNAL(
        "v178_global_performance_installed", int(OWNER_ID or 0) or None,
