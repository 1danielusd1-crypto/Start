...
Семантическая карта функций. Зафиксированная пользователем ветка не должна ломаться последующими правками; допускается рефакторинг только при сохранении контракта и регрессионных тестов.
...
- **Чаты · реальные имена / полная проверка** (`ui.chat_identity`, contract r1) — Хранить и показывать реальную Telegram-идентичность каждого известного чата и по кнопке полностью синхронизировать доступные изменения. Зависимости: forward.core, multitenant.core.
...
### 🩺 Диагностика
- **Журналы / диагностика** (`diagnostics.journal`, contract r3) — Фиксировать ошибки, события, скорость и давать скачиваемые журналы. Зависимости: storage.mega.
