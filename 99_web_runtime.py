...
# v163: BOT TOKEN must never be part of the public webhook URL.
import hashlib as _v163_webhook_hashlib

_v163_webhook_seed = (
    os.getenv("WEBHOOK_SECRET", "").strip()
    or _v163_webhook_hashlib.sha256(
        ("telegram-webhook-v163|" + str(BOT_TOKEN) + "|" + str(os.getenv("RENDER_SERVICE_ID", "") or WEBHOOK_URL)).encode("utf-8")
    ).hexdigest()[:40]
)
...
def _v177_legacy_0269_set_webhook():
    ...
    try:
        ...
        # Compatibility fallback for an older pyTelegramBotAPI. URL is still token-free.
