# DATA CONSTITUTION v188 — Telegram Bot
...
## v188 — two restore modes are strictly separated
...
- `🩹 Аккуратное восстановление` is not a restore engine. It is a RAM-only manual replay helper. While enabled, only manually forwarded Telegram finance messages are assigned to the day currently open in the main finance window.
...
- Automatic forwarding and ordinary fresh finance messages keep normal date semantics.
- DATA CONSTITUTION ledger/snapshot/checkpoint protections remain active for every newly replayed finance operation.


## v188 restore forwarding invariant
