...
"""v176: owner process control center + callback latency diagnostics.
...
VERSION = "bot_v176_process_control_center"
V176_FILE_MARKER = "v176_process_control_center"
...
try:
    _V176_MIGRATED_HEAVY = bool(heavy_processes_enabled_v175())
except Exception:
...
_V176_PROCESS_DEFS = {
    ...
    "win_diag": ("ui", "🩺 Диагностика окон", _V176_MIGRATED_HEAVY, "hot", "Маркировка/диагностика жизненного цикла окон."),
    ...
    "btn_chain": ("ui", "🧾 Трассировка press/result", True, "hot", "Две дополнительные записи на обработанный callback."),
    "btn_press": ("ui", "📝 Журнал button_pressed", True, "hot", "Запись исходной кнопки в общий журнал."),
    "win_journal": ("ui", "📐 Window-журнал", True, "hot", "Подробные window_* события, которые раньше писались принудительно."),
    ...
    "failed_repair": ("mega", "🩹 Repair failed-задач", _V176_MIGRATED_HEAVY, "medium", "Автоматический безопасный ремонт failed durable tasks."),
    "failed_diag": ("mega", "🔬 Диагностика failed-задач", _V176_MIGRATED_HEAVY, "medium", "Фоновый диагностический скан failed tasks."),
    "mega_maint": ("mega", "🧰 MEGA migration/cleanup", True, "hot", "v153 migration и remote runtime cleanup; старый v175 light mode это не отключал."),
...
def heavy_processes_status_v175() -> str:
    ...
    return f"⚙️ Центр процессов v176: отключено {off} из {len(_V176_PROCESS_DEFS)} управляемых процессов."
...
_V176_ORIG_V161_EDIT_RETRY = globals().get("_v161_edit_retry")
...
def bot_journal(action: str, chat_id=None, detail: str = "", level: str = "INFO"):
    ...
    if str(level or "INFO").upper() not in {"ERROR", "CRITICAL"}:
    ...
    if callable(_V176_ORIG_BOT_JOURNAL):
        return _V176_ORIG_BOT_JOURNAL(action_s, chat_id, detail, level)
    return None


# Fast UI experiment: OFF removes the outer v161 3-attempt retry loop. v160's one short 429 retry remains.
def _v161_edit_retry(chat_id: int, message_id: int, text: str, reply_markup=None, parse_mode=None, purpose: str = "ui") -> str:
    if v176_process_enabled("ui_retry") and callable(_V176_ORIG_V161_EDIT_RETRY):
        return _V176_ORIG_V161_EDIT_RETRY(chat_id, message_id, text, reply_markup=reply_markup, parse_mode=parse_mode, purpose=purpose)
    try:
        return str(fast_ui_edit_message_text(int(chat_id), int(message_id), text, reply_markup=reply_markup, parse_mode=parse_mode, purpose=purpose) or "failed")
    except Exception:
        return "failed"
...
if callable(_V176_ORIG_FIN_REFRESH):
    def schedule_financial_window_refresh(*args, **kwargs):
        if not v176_process_enabled("fin_refresh"):
            return False
        return _V176_ORIG_FIN_REFRESH(*args, **kwargs)
...
if callable(_V176_ORIG_RUNTIME_HEARTBEAT):
    def _runtime_heartbeat_job():
        if not v176_process_enabled("runtime_upload"):
            return None
        return _V176_ORIG_RUNTIME_HEARTBEAT()
...
def _v176_cancel_for(code: str) -> None:
    ...
    keys = {
        ...
        "mega_maint": ["v153-mega-migration", "v153-mega-migration-next", "v153-mega-migration-retry", "v153-runtime-cleanup"],
...
def _v176_resume_for(code: str) -> None:
    ...
    try:
        ...
        elif code == "mega_maint" and callable(globals().get("_v153_schedule_migration")):
            scheduler.schedule("v153-mega-migration", 2.0, globals()["_v153_schedule_migration"])
...
def _v176_speed_text() -> str:
    ...
    lines = ["📊 СКОРОСТЬ КНОПОК v176", "", "Замеряется полное время реального callback-handler, включая Telegram edit/retry."]
    if not s.get("count"):
        lines += ["", "Пока нет замеров после запуска v176."]
    else:
        lines += [
            "",
    ...
    lines += ["", "Для чистого сравнения: сделай 10–20 одинаковых переходов, запомни P50/P90, затем выключи один процесс и повтори."]
    return "\n".join(lines)[:3900]
...
def _v176_menu_text(page: str = "ui") -> str:
    ...
    lines = [
        "⚙️ ЦЕНТР ПРОЦЕССОВ / СКОРОСТЬ v176",
...
def _v176_speed_keyboard():
    kb = types.InlineKeyboardMarkup()
    kb.row(IB("🔄 Обновить замер", callback_data="v176:speed"))
    kb.row(IB("⬅️ Процессы", callback_data="v176:menu"), IB("✖️ Закрыть", callback_data="info_close"))
    return kb


# INFO integration: replace the old direct v175 toggle with one process-center entry.
_V176_PREV_INFO_TEXT = globals().get("build_info_text")
_V176_PREV_INFO_KB = globals().get("build_info_keyboard")

def build_info_text(chat_id: int, *args, **kwargs) -> str:
    try:
        base = str(_V176_PREV_INFO_TEXT(int(chat_id), *args, **kwargs) if callable(_V176_PREV_INFO_TEXT) else "")
    except TypeError:
        base = str(_V176_PREV_INFO_TEXT(int(chat_id)) if callable(_V176_PREV_INFO_TEXT) else "")
    if int(chat_id) == int(OWNER_ID or 0):
        rows = [r for r in base.splitlines() if not r.strip().startswith(("🧱 Тяжёлые процессы:", "⚡ Тяжёлые процессы:"))]
...
def build_info_keyboard(chat_id: int):
    kb = _V176_PREV_INFO_KB(int(chat_id)) if callable(_V176_PREV_INFO_KB) else types.InlineKeyboardMarkup()
    if int(chat_id) != int(OWNER_ID or 0):
        return kb
    try:
        ...
        if not found:
            ...
            for idx, row in enumerate(rows):
                labels = " ".join(str(getattr(b, "text", "") or "") for b in (row or [])).casefold()
                if "назад" in labels or "закры" in labels:
                    insert_at = idx
                    break
...
def _v176_register_callback() -> int:
    try:
        bot.callback_query_handler(func=_v176_filter)(_v176_callback)
...
def _v176_install_perf_wrappers() -> int:
    ...
    for handler in list(getattr(bot, "callback_query_handlers", []) or []):
        ...
        if not callable(original) or getattr(original, "_v176_perf_wrapped", False):
            continue
        def _wrapped(call, _orig=original):
            ...
            try:
                ...
                action = str(resolver(raw) or raw) if callable(resolver) else raw
            except Exception:
                action = raw
            try:
                return _orig(call)
                ...
                try:
                    _V176_PERF.append({"ts": _v176_time.time(), "action": action[:120], "elapsed": max(0.0, _v176_time.monotonic() - started)})
                except Exception:
                    pass
        _wrapped._v176_perf_wrapped = True
...
def _v153_validate_restore_gz(gz_path: str):
    ...
    folder = _tempfile.mkdtemp(prefix="v176_restore_validate_")
    ...
    try:
        ...
        if not export_version.startswith(tuple(f"bot_v{i}_" for i in range(153, 177))):
...
_v176_root()
_v176_apply_runtime_flags()
_V176_CALLBACK = _v176_register_callback()
_V176_PERF_WRAPPERS = _v176_install_perf_wrappers()
try:
    _V176_ORIG_BOT_JOURNAL(
        "v176_process_control_installed", int(OWNER_ID or 0) or None,
        f"managed={len(_V176_PROCESS_DEFS)} callback={_V176_CALLBACK} perf_wrappers={_V176_PERF_WRAPPERS} migrated_heavy={int(_V176_MIGRATED_HEAVY)}",
    )
except Exception:
    pass
# v176_process_control_center
