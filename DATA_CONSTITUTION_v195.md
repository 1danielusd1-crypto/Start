# DATA CONSTITUTION v195 — MEGA LIGHT / BALANCE SOURCE TRUTH / CHAT IDENTITY
...
## 13. v194: единый источник финансового остатка
`records` и `daily_records` — не две независимые бухгалтерии. `records` является записываемым ledger, `daily_records` — дневным индексом.

Правила v194:
- LOW-RAM flush только сохраняет уже загруженные cold-поля и не имеет права перестраивать `daily_records` просто потому, что `records` был прочитан;
- финансовая нормализация при найденной рассинхронизации делает дедуплицированное lossless reconciliation двух индексов, а затем заново строит дневной индекс;
- ARS окно дня, «остаток после каждого расхода», общий итог, `/balance` и ARS financial-view totals читают один и тот же дневной индекс;
- CSV продолжает формироваться из дневного индекса; Excel/XLSX/Google export получает reconciled ledger, поэтому история не расходится между Telegram и таблицами;
- открытие вспомогательного окна является read-only действием и не может менять бухгалтерские данные;
- режим «с гомонковыми» показывает полный остаток, а режим «без гомонковых» исключает резерв.

При обнаружении orphan-строк между индексами бот пишет `finance_index_reconciled_v194` в журнал.


## 14. v195: идентичность чата отделена от прав доступа
- Имя/название Telegram-чата является свойством самого chat_id и его Telegram-карточки (`info.title` / `info.username`).
- Режим владельца, финансовый режим, пересылка, круг/пространство и любые роли не имеют права переименовывать чат или подменять его меткой владельца.
- Метка `🏀` относится только к точному приватному `OWNER_ID`.
- Владелец-доступ отображается отдельным UI-бейджем `☑️`/`⬜` рядом с неизменным названием.
- Одинаковый Telegram identity (id/username) в picker показывается один раз. Одинаковый title без доказательства общей identity не склеивается.
- Эти правила являются UI/identity-слоем и не меняют §§1–13: strict restore, MEGA durability, таблицы, ARS/USD и единый источник остатка остаются неизменными.
